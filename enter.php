...
if (isset($_POST['name']) && isset($_POST['city'])) {
    ...
    $preference = sanitizeMySQL($conn, $_POST['preference']);
    $best = sanitizeMySQL($conn, $_POST['best']);


    // the prepared statement - note: 6 question marks represent
    // 6 variables we will send to database separately
    $query = "INSERT INTO coffeehouse (name, city, preference, best)
    VALUES (?, ?, ?, ?)";
    ...
    if ( $stmt = mysqli_prepare($conn, $query) ) {
        ...
        // i - integer, s - string/text , d - double (decimal), b - blob
